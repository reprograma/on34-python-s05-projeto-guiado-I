...
def main():
    ...
    while True:
        ...
        elif cod_opcao == 6: print("Programa encerrado"); break
...
def incluir_nova_aluna():
    ...
    try:
        notas = list(map(float, input("Digite as notas da aluna (separadas vírgula): ").split(",")))
        presencas = list(map(lambda x: x.lower() == 'true', input("Digite a presença da aluna (True/False separados por vírgula): ").split(",")))
        participacao = float(input("Digite a nota de participação da aluna: "))
    except ValueError:
        print("Entrada inválida. Tente novamente com dados válidos.")
        return

    if any(nota < 0 or nota > 10 for nota in notas):
        print("Entrada inválida. As notas devem estar entre 0 e 10.")
        return
...
def consultar_lista_alunas():
    if not dataset:
        print("Não há alunas cadastradas no sistema.")
    else:
